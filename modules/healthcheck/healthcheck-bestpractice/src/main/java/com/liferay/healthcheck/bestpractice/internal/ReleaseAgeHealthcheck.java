...
public class ReleaseAgeHealthcheck implements Healthcheck {
	...
	public Collection<HealthcheckItem> check(long companyId) throws Exception {
		...
		System.out.println("found " + productGroupVersionInformation.size() + " entries for this product group (" + currentReleaseInformation.productGroupVersion + ")");
		System.out.println("This version's release date: " + currentReleaseInformation.getReleaseDate() + " (" + currentReleaseInformation.productVersion + ")");
		ReleaseInformation latestVersion = productGroupVersionInformation.iterator().next();
		System.out.println("Latest version's release date: " + latestVersion.getReleaseDate() + " (" + latestVersion.productVersion + ")");
		...
		if(ReleaseInfo.isDXP()) {
			if(releasesBack > 0) { 
				...
				if(immediateNextRelease != null && _config.ageInWeeksBeforeUpdateInstalled() >= 0) {
					...
					System.out.println("Immediate next release: " + immediateNextRelease.productVersion + " available since " + immediateNextRelease.getReleaseDate());
					System.out.println("Latest release: " + latestVersion.productVersion + " available since " + latestVersion.getReleaseDate());
					result.add(
						new HealthcheckItem(
