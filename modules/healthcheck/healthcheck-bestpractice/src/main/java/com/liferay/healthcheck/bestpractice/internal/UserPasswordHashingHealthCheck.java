 ...
 * algorithm that's actually in use.
 * Liferay will upgrade the password hash during log in, when it has 
 * access to the clear-text password - simply logging in (locally, not on 
 * SSO) is sufficient to update the hash
 *
 * @author Olaf Kock
 */
@Component(configurationPid = "", service = Healthcheck.class)
public class UserPasswordHashingHealthCheck implements Healthcheck {
	...
	public Collection<HealthcheckItem> check(long companyId)
		...
		if (hashingAlgorithm.startsWith(_BCRYPT)) {
			...
			result.add(
				new HealthcheckItem(
					workFactor>=_OWASP_BCRYPT_WORKFACTOR, _ADDITIONAL_DOCUMENTATION, null,
					"default-hashing-bcrypt-should-use-workfactor-x-detected-x", _OWASP_BCRYPT_WORKFACTOR, hashingAlgorithm));
			...
			result.add(
				new HealthcheckItem(
					workFactor >= _OWASP_PBKDF2_WITH_HMAC_SHA1_WORKFACTOR, _ADDITIONAL_DOCUMENTATION, null,
						"default-hashing-pbkdf2withhmacsha1-should-use-workfactor-x-detected-x", _OWASP_PBKDF2_WITH_HMAC_SHA1_WORKFACTOR, hashingAlgorithm));
					
		} else if (hashingAlgorithm.startsWith("MD") ||
				 hashingAlgorithm.startsWith("SHA") ||
				 hashingAlgorithm.startsWith("NONE")) {

			result.add(
				new HealthcheckItem(
					false, _ADDITIONAL_DOCUMENTATION, null,
					"insecure-hashing-algorithm-x", hashingAlgorithm));
		} else {
			result.add(
				new HealthcheckItem(
					false, _ADDITIONAL_DOCUMENTATION, null,
					"unknown-hashing-algorithm-x", hashingAlgorithm));
		}

		HealthcheckBestPracticeConfiguration
			healthcheckBestPracticeConfiguration = null;

		if (companyId != CompanyConstants.SYSTEM) {
			healthcheckBestPracticeConfiguration =
				_configurationProvider.getCompanyConfiguration(
					HealthcheckBestPracticeConfiguration.class, companyId);
		}
		else {
			healthcheckBestPracticeConfiguration =
		...
		long usersCount = Math.min(
			_userLocalService.getUsersCount(
				companyId, WorkflowConstants.STATUS_APPROVED),
			healthcheckBestPracticeConfiguration.
				maxUsersTestedForHashingAlgorithm());
		...
		for (int i = 0; i < (usersCount / pageSize); i++) {
			...
			try {
				users = _userLocalService.getUsers(
					...
					(i + 1) * pageSize,
					UserEmailAddressComparator.getInstance(true));

				for (User user : users) {
					String pwd = user.getPassword();

					String algorithm = CompatPasswordEncryptorUtil.
							getEncryptedPasswordAlgorithmSettings(pwd);
		...
		Set<String> whitelistedAlgorithms = new HashSet<>(
			Arrays.asList(
				healthcheckBestPracticeConfiguration.
					passwordHashingAlgorithmWhitelist()));
		...
		for (HashMap.Entry<String, Long> entry : algorithms.entrySet()) {
			if (entry.getKey(
				...
				result.add(
					new HealthcheckItem(
						entry.getValue() > 0, _ADDITIONAL_DOCUMENTATION, _LINK,
						"x-out-of-x-users-use-default-hashing-algorithm-x",
						entry.getValue(), usersCount, hashingAlgorithm));
			}
			else if (whitelistedAlgorithms.contains(entry.getKey())) {
				result.add(
					new HealthcheckItem(
						true, _ADDITIONAL_DOCUMENTATION, _LINK,
						"x-out-of-x-users-use-whitelisted-hashing-algorithm-x",
						entry.getValue(), usersCount, entry.getKey()));
			}
			else {
				result.add(
					new HealthcheckItem(
						false, _ADDITIONAL_DOCUMENTATION, _LINK,
						"x-out-of-x-users-dont-use-hashing-algorithm-x-but-x",
						entry.getValue(), usersCount, hashingAlgorithm,
						entry.getKey()));
			}
		}
